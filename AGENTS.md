...
* **Haukku luetaan `readBarkFromGatewayItem`-funktiolla, ei käsin.** Gatewayt ilmaisevat haukun neljällä eri tavalla: elävänä lippuna (`isBarking` tai `barking`), tahtina (`barkRate`), vapaana `alarm`-merkkijonona tai **viimeisimmän haukun aikaleimana** (`lastBarkTime`). Aikaleima on näistä luotettavin, koska haukku on hetkellinen ja 5 sekunnin pollaus ehtii usein nähdä elävän lipun jo sammuneena. Aikaleima kelpuutetaan uudeksi haukuksi vain jos se on sekä edellistä tuoreempi että enintään `BARK_EVENT_MAX_AGE_MS` vanha — ilman ikäehtoa sovelluksen avaaminen pannan vieressä laukaisisi hälytyksen tunteja vanhasta haukusta. `alarm`-kentän tulkinta on tahallisen kapea (vain bark/haukk/sound): **väärä haukkuhälytys on pahempi kuin puuttuva**, koska se lähettää metsästäjän äänettömän koiran perään. Älä laajenna ehtoa muihin hälytystyyppeihin.
...
* **Poistolla on oltava vastinpari: elvytys.** Poistorekisteri (`deletedDogIds`) on sekä palvelimella että jokaisen selaimen localStoragessa, ja se on luonteeltaan **yksisuuntainen**. Ilman elvytysviestiä (`revivedDogIds`) kerran poistettua pantaa ei voisi koskaan lisätä takaisin: palvelin jatkaisi poiston lähettämistä ja jokainen client soveltaisi sen uudelleen, jolloin lisätty koira katoaisi noin 2,5 sekunnissa sekä kartalta että listalta. Kun lisäät koiran, `handleAddDog` lähettää kyseisen koiran tunnisteet (`getDogIdentifiers`) elvytyslistana. Kolme sääntöä joita ei saa rikkoa: (1) tunnisteiden muunnelmat on tuotettava `deletedIdVariants`-funktiolla, jotta poisto ja elvytys poistavat täsmälleen samat merkkijonot; (2) **poisto voittaa elvytyksen** — sekä palvelin (`clearRevivedOnServer`) että client (sama lähetys sisältää molemmat) tulkitsevat uudemman poiston voittajaksi, muuten pian lisäämisen jälkeen tehty poisto kumoutuisi; (3) elvytys on hetkellinen tapahtuma, joka kulkee **vain pikakanavalla** (relay), ei koskaan tallennettavana tilana, koska vanhentunut elvytysmerkintä kumoaisi myöhemmän poiston samassa dokumentissa ilman järjestystietoa.
